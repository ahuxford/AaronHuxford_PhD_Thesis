...
for i,fname in enumerate(fnames_sep):
    ...
    if fname=='ic_v8':
        ...
        N_sum = 1339 #1093  # dt half of others
    else:
...
plt.figure(1)
plt.plot(labels_over,Psum_over,linewidth=5,marker='.',markersize=20,label='Domain overlapping',color='tab:red')
...
plt.figure(2)
plt.plot(labels_sep,Psum_sep,linewidth=5,marker='.',markersize=20,label='Domain decomposition',color='tab:green')
plt.plot(labels_over,Psum_over,linewidth=5,marker='.',markersize=20,label='Domain overlapping',color='tab:red')
